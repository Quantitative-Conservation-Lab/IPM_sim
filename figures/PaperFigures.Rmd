...
# 
# 
# write.csv(all.meds, here::here('results', 'Processed csvs', 'all.meds.csv'), row.names = F)

all.meds <- read.csv(file = here::here('results', 'Processed csvs', 'all.meds.csv'), 
...
#use me fig 1
ggplot(rel.bias.few, aes(x = factor(det.abund), y = variable, fill = bias)) +
  geom_tile(color = 'grey50') +
...
ggplot(rel.bias.few, aes(x = det.abund, y = bias, col = factor(variable), group = factor(variable))) +
  geom_point() + geom_line() +
  geom_hline(aes(yintercept = 0), linetype = 'dotted') +
  xlab('Count detection') + ylab('Relative bias') +
...
all.meds.sc <- read.csv(file = here::here('results', 'Processed csvs', 'all.meds.sc.csv'), 
                     header = T, stringsAsFactors = F)
...
## dot plot
# ggplot(plot.vals, aes(x = fec_cat, y = value, col = factor(dataset), group = factor(dataset))) +
#   geom_point() + geom_line() +
#   geom_hline(aes(yintercept = 0), linetype = 'dotted') +
#   xlab('True fecundity') + ylab('Relative bias') +
#   facet_grid(variable~phi1_cat, scales = 'free') +
#   theme_bw() +
#   theme(legend.position = 'top',
#              plot.subtitle = element_text(size = 10, hjust = 0.5, vjust = 1)) +
#   scale_color_manual(values = rainbow2[-c(1,4)], name = '')
...
#use me fig 3
ggplot(rmse.few, aes(x = factor(det.abund), y = variable, fill = rmse)) +
  geom_tile(color = 'grey50') +
  xlab('Count detection') + ylab('') +
  facet_grid(dataset ~ lambda.scenario, drop = T, scales = 'free_x', labeller = label_parsed) +
  scale_fill_gradient2(name = "RMSE",
...
ggplot(rmse.few, aes(x = det.abund, y = rmse, col = factor(variable), group = factor(variable))) +
  geom_point() + geom_line() +
  geom_hline(aes(yintercept = 0), linetype = 'dotted') +
