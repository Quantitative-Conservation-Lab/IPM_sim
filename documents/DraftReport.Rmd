...
#for investigating, particularly scenarios where survivals are low
sc <- c(9)
niter <- 25
...
true_vals <- data.frame(scenario = rep(1:15, times = 6),
                        variable = rep(c('mean.phi.1.', 'mean.phi.2.', 'mean.p', 'fec', 'p.surv', 'phi.nest'), 
                                       each = 15),
                        value = c(rep(c(0.8, 0.3, 0.8, 0.3, 0.5), times = 3), #adult phi
                                  rep(c(0.7, 0.2, 0.7, 0.2, 0.4), times = 3), #juv phi
                                  c(0.8, 0.8, 0.3, 0.3, 0.5, rep(NA, 5), c(0.8, 0.8, 0.3, 0.3, 0.5)), #mean.p
                                  rep(NA, 15), #fec: what goes here?
                                  rep(c(0.85, 0.85, 0.3, 0.3, 0.5), times = 3), #p.surv
                                  rep(c(0.995, 0.975, 0.995, 0.975, 0.99), times = 3))) #phi.nest

ggplot(post.meds, aes(y = value, x = scenario, group = scenario)) +
  geom_boxplot() +
  geom_point(size = 0.7, col = 'darkgrey') + 
  geom_point(data = true_vals, aes(y = value, x = scenario, group = scenario), col = 'darkred') +
