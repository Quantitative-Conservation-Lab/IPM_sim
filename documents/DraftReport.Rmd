...
```{r figures - multiple chains, results = 'hide', message = FALSE}
...
Understanding the processes that control population dynamics in wild populations is a critical component of conservation but is frequently constrained by missing demographic information, uncertainty in data, and temporally or spatially misaligned datasets (Schaub & Abadi 2011, Zipkin & Saunders 2017). Integrated population models (IPMs) are an increasingly popular tool in ecology that offer release from these limitations by providing a framework to combine disparate datasets in a single, unified analysis (Schaub & Abadi 2011). This approach can provide several advantages like estimation of otherwise non-identifiable parameters, improved precision in parameter estimation, and [spatio-temporal inference] (citation). These advances have practical benefits for resource managers that must make decisions based on limited available information. As IPMs become more widely used, however, it is critical that they are robustly evaluated. 

Since the introduction of IPMs to ecology in the early 2000s and following the work of Besbeas and colleagues with passerines (Besbeas et al. 2002, 2005), the common approach has been to estimate parameters by combining mark-recapture and abundance data into a joint likelihood. Researchers have has since expanded the use of IPMs to include a variety of taxa (e.g. in bats Rhinolophus ferrumequinum: Schaub et al. 2007, in Svalbard reindeer Rangifer tarandus platyrhynchus: Lee et al. 2015, Antarctic toothfish Dissostichus mawsoni: Mormede et al. 2019, wild boar Sus scrofa: Gamelon et al. 2019), [include multivariate observations with estimation of transition probabilities within multi-state and multi-site systems] (McCrea et al. 2012), evaluate management efforts (Cruz et al. 2018), and to include environmental processes and stochasticity (Oppel et al. 2014, Woodworth et al. 2017). This growth in usage has occurred despite a relatively poor understanding of how to assess goodness of fit however, and we lack knowledge of the limits of the IPM framework in situations where available data are sparse, of poor quality, or when certain data sources are absent. For example, many field studies may have survey and reproductive data while missing survival estimates from mark-recapture datasets. 

Data simulation studies provide an avenue for robust evaluation of the IPM framework, but few such studies exist. Those that have used data simulation largely focus on proof of 1) concept (i.e. improved precision and reduced bias McCrea et al 2010, Chandler & Clark 2014, Schaub & Fletcher 2015, Bled et al. 2017, Ahrestani et al. 2017, Bowler et al. 2019), and 2) model assumptions such as independence of datasets (Abadi et al. 2010, Schaub & Fletcher 2015, Weegman et al. 2020). Riecke et al. (2019) found that parameter estimates from IPMs were highly sensitive to violations of mark-recapture model assumptions (i.e. marker-induced bias and survival heterogeneity). These findings further motivate studies to determine the reliability of IPMs in parameter estimation and in detecting trends when survival is not informed by data or when a data source is sparse, biased, or highly uncertain. 

We aim to improve our understanding of this important statistical approach, particularly for passerine bird life history. In this study, we simulate data to examine IPM performance across a range of scenarios where data informing key demographic parameters are sparse or missing. We simulate three datasets for a for a theoretical passerine: 1) count surveys, 2) reproductive output, and 3) mark-recapture (MR) of banded individuals. We then run integrated models systematically 1) omitting nest or MR data, 2) varying the detection probabilities of the abundance and MR datasets, and 3) creating scenarios where demographic parameters like survival and fecundity are high or low. By examining model performance, we aim to identify scenarios where IPMs do not lead to improved precision or cannot estimate unidentifiable parameters. In doing so, we refine our understanding of the mechanisms underlying IPMs and can better identify when they may serve as a useful tool. 
