...
#filter to see that adult survival bias does in fact vary with L/M/H detection
ggplot(phiad.plot.vals %>% filter(variable == 'Adult survival'), 
       aes(x = factor(phiad_cat), y = variable, fill = value)) +
  geom_tile(color = 'grey50') +
  xlab('True Adult survival') + ylab('Parameter') +
  facet_grid(~lambda.scenario+simscenarios, drop = T, scales = 'free_x') +
    scale_fill_gradient2(name = "Relative bias",
