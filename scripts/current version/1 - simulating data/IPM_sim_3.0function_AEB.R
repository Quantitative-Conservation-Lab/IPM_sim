...
simPopTrajectory <- function(n.years, age.init,
  ...
  for(t in 1:n.years){
    ...
    # i think this condition should always be met as long as f > 0 (AEB)
    if(inpop[t+1]>inpop[t]){ # if pop is growing
      indfates[3,t,(inpop[t]+1):inpop[t+1]]<-1 # add chicks to 
...
simData <- function(indfates, n.years,
  ...
  if(!is.na(p.1) & !is.na(p.ad)) {
    ...
    # TODO
    # OK i'm thinking thru this
      # shouldn't we also look to see if the individual was previously banded as a chick?
    
    # AEB - what is this doing? this will return 0 and isn't used
    ...
    for(i in 1:dim(ind_mr.a)[3]){
      if(length(which(!is.na(ind_mr.a[1:2,,i])))==0){
        ...
        #rm[i]<-0
        # aeb - changing here to only mark a proportion of population
        # but everyone that gets marked has state known at first cap (see change below)
        rm[i] <- 1 - rbinom(1,1,p.ad) # only mark some proportion of those possible
      }
    }
    if(sum(rm>0)){
    ...
    for(i in 1:mr_ind.a){
      # aeb - why take 1,1 and 1,2. what goes on with second row?
      # oh i see, this only comes up if it was first seen as an adult
    ...
    # AEB - ok this all looks good thru here
    
    #code to track the ages, 1 is for chicks, 2 1years, 3 adults (code not currently in use)
    #AEB - are these ages correct?? ^^
    #IND_MR only has the 1 year olds and adults
    ...
    if(sum(rm_2)>0){
      ...
      add_age_chtrue.a[-(which(rm_2==1)),] # AEB - why printing?
    } else {}
    ...
    for(t in 1:n.years){
      ...
      for(i in 1:length(IND_MR[1,1,])){ #no.animax
        # AEB - why is the second statement needed?
          # oh, this prevents individuals that were first seen as adults
        if(!is.na(IND_Nest[3,t,i]) & length(which(is.na(IND_Nest[1:2,t,i])))==2){
          #tomark.j[t,i]<-1
          tomark.j[t,i]<- rbinom(1,1,p.1) # only mark some proportion of those possible
        }  
      }
    }
    ...
    for(i in 1:mr_ind.j){
      ...
      # AEB - shouldn't it be this below??
      # TODO check - i changed thru here
      g <- which(!is.na(indfates[3,,indsmarked.j[i]]))
      age.j[i] <- 3 #at marking (1=1year olds, 2=adults, 3= chicks)
      first.j[i] <- g[1] 
      h <- which(indfates[3,1:n.years,indsmarked.j[i]]==1) 
      last.j[i] <- max(h)
    }
    ...
    # AEB - shouldn't the chicks be age 3, 1, 2, 2, 2, ... always?
    ...
    for(i in 1:mr_ind.j){
      if(age.j[i]==3){ #hatchlings
        # we've already decided these are going to get marked
        # so we don't need the p here...condition on first capture
