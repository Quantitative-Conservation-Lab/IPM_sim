...
foreach(i = 2611:2630) %dopar% { # loop over replicate sims  #####
  library(here)
  library(nimble)
  library(IPMbook)
  library(popbio)
  ...
  # for (d in 2:nrow(dem_scenarios)) {
  # for (d in 8:9) {
    for (d in c(1,3,6,7,9)) {
    ...
    # for (s in 1:nrow(surv_scenarios)) {
      # for (s in c(1,3,5,9,11,13,17,19,21)) { #IPM scenarios
    for (s in c(7,8,15,16,23,24)) { #no MR scenarios
    # for (s in c(25,27,29)) { #no prod scenarios
      # for (s in c(31:32)) {
